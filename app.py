...
if check_password():
    ...
    st.title("🛰️ Inteligencia Satelital BioCore V5")

    # PESTAÑAS SIEMPRE VISIBLES
    tab_mapa, tab_hist, tab_clima = st.tabs(["🌍 Monitoreo y Mapa", "📊 Historial Landsat", "🌡️ Clima y Fuego"])
    ...
    if btn_ejecutar:
        for nombre, info in CLIENTES.items():
            ...
            with tab_mapa:
                ...
                c3.metric("Estado Técnico", estado)

            # PESTAÑA 2: HISTÓRICO
            with tab_hist:
                st.subheader(f"📈 Tendencia Landsat: {nombre}")
                st.line_chart(pd.DataFrame([res['savi']*0.9, res['savi']*1.1, res['savi']], columns=["Índice"]))

            # PESTAÑA 3: CLIMA Y FUEGO
            with tab_clima:
                st.subheader(f"🌡️ Entorno: {nombre}")
                ...
                if fire.size().getInfo() > 0:
                    st.error(f"🔥 Alerta de Fuego detectada en {nombre}")
                else:
                    st.success(f"✅ Zona libre de incendios: {nombre}")

        st.balloons()
    else:
        # Mensajes cuando NO se ha presionado el botón
        with tab_hist:
            st.info("Presiona 'Iniciar Escaneo' en la barra lateral para ver el historial Landsat.")
        with tab_clima:
            st.info("Presiona 'Iniciar Escaneo' para cargar datos de TerraClimate y FIRMS.")
