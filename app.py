...
# --- 2. BASE DE DATOS DE CLIENTES Y PARÁMETROS ---
UMBRAL_CRITICO = 0.40  
CLIENTES_DB = {
...
def procesar_datos_limpios(sheet_id, pestaña):
    try:
        ...
        # Columnas a procesar (unificando nombres para el reporte)
        indices = ["SAVI", "NDWI", "SWIR", "Arcillas", "Deficit"]
        presentes = []
        for col in indices:
            if col in df.columns:
                # Forzar a número para evitar errores con "Muy Alto"
...
def generar_graficos_cascada(df, columnas):
    ...
    colores = {"SAVI": "#143654", "NDWI": "#2E7D32", "SWIR": "#D4AC0D", "Deficit": "#C62828"}
...
# --- 5. REPORTE PDF (DISEÑO PROFESIONAL) ---
...
def crear_pdf_auditoria(df, proyecto, columnas):
    info = CLIENTES_DB[proyecto]
    ultimo_savi = df.iloc[-1].get('SAVI', 0)
    es_alerta = ultimo_savi < UMBRAL_CRITICO
    ...
    msg = f"Se detecta un valor de {ultimo_savi:.4f}. Desviación crítica." if es_alerta else "Valores estables."
    pdf.multi_cell(0, 7, f"1. HALLAZGO: {msg}", border=1)
    pdf.multi_cell(0, 7, "2. RIESGO: Posible alteración de masa criosférica y arrastre de material.", border=1)
    pdf.multi_cell(0, 7, "3. RECOMENDACIÓN: Ejecutar inspección de terreno de manera inmediata.", border=1)
    ...
    pdf.image("temp_p.png", x=15, w=180)
...
with c_info:
    ...
    st.info(f"**Contacto:** {info_p['contacto']}\n\n**Región:** {info_p['region']}\n\n**Sensores:** {info_p['sensores']}")

with c_map:
    # Solución al error de tiles: Usamos Esri World Imagery con atribución
    m = folium.Map(location=[info_p['lat'], info_p['lon']], zoom_start=12, 
                   tiles='https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',
                   attr='Esri World Imagery')
    folium.Marker([info_p['lat'], info_p['lon']], popup=proyecto_sel, icon=folium.Icon(color='blue')).add_to(m)
    folium_static(m)

if st.button("🚀 INICIAR AUDITORÍA DE CUMPLIMIENTO"):
    with st.spinner("Procesando datos..."):
        df_f, cols_f = procesar_datos_limpios(info_p['sheet_id'], info_p['pestaña'])
        
        if not df_f.empty:
            # Métricas
            st.divider()
            m1, m2, m3 = st.columns(3)
            val_final = df_f.iloc[-1].get('SAVI', 0)
            m1.metric("SAVI/NDSI Actual", f"{val_final:.4f}")
            m2.metric("Déficit de Cobertura", f"{df_f.iloc[-1].get('Deficit', 0):.0f}")
            m3.metric("Estatus", "ALERTA" if val_final < UMBRAL_CRITICO else "NORMAL")

            # Gráficos
            st.image(generar_graficos_cascada(df_f, cols_f))
            
            # Descarga PDF
            pdf_bytes = crear_pdf_auditoria(df_f, proyecto_sel, cols_f)
            b64 = base64.b64encode(pdf_bytes).decode()
            href = f'<a href="data:application/pdf;base64,{b64}" download="Auditoria_BioCore.pdf" style="display:block; text-align:center; padding:15px; background-color:#183654; color:white; border-radius:10px; text-decoration:none; font-weight:bold; font-size:18px;">📄 DESCARGAR AUDITORÍA TÉCNICA (PDF)</a>'
