...
# Nota: En una versión avanzada, esta lista también podría vivir en un Sheet maestro.
CLIENTES_DB = {
...
def crear_excel_automatico(nombre):
    ...
    try:
        ...
        # Encabezados estándar para tus scripts
        hoja_activa.append_row(["Fecha", "NDSI", "NDWI", "SWIR", "Polvo", "Deficit"])
        # Compartir contigo para que lo veas en tu Drive
        # nueva_hoja.share('tu_correo@gmail.com', perm_type='user', role='editor') 
        return nueva_hoja.id
    except Exception as e:
        return str(e)

def obtener_datos(sheet_id, pestaña):
    """Extrae datos de Google Sheets y los convierte a DataFrame"""
    try:
...
if opcion == "➕ Registrar Proyecto":
    ...
    with st.form("form_nuevo"):
        ...
        if submit and nuevo_nom:
            with st.spinner("Generando archivo en la nube..."):
                ...
                st.warning("Copia este ID y agrégalo a tu script 'monitor_biocore.py' para empezar el monitoreo.")
    ...
    if st.button("🔄 Sincronizar Datos"):
        with st.spinner("Leyendo Google Sheets..."):
            ...
            if not df.empty:
                ...
                st.error("No se encontraron datos en la nube para este proyecto.")
    ...
    if f"df_{proyecto_sel}" in st.session_state:
        ...
        with col1:
            ...
            ultimos = df.tail(1)
            for col in df.columns[1:]: # Omitir fecha
        ...
        with col2:
            ...
            # Graficar todos los índices relevantes
