import streamlit as st
import json
import ee
import requests
from datetime import datetime
from googleapiclient.discovery import build
from google.oauth2 import service_account
from streamlit_folium import folium_static
import folium

# --- 1. CONFIGURACIÓN E IDENTIDAD ---
st.set_page_config(page_title="BioCore V5", layout="wide")

T_TOKEN = st.secrets["telegram"]["token"]
T_ID = st.secrets["telegram"]["chat_id"]
UMBRAL = 0.4

CLIENTES = {
    "Laguna Señoraza (Laja)": {
        "coords": [[-72.715,-37.275],[-72.715,-37.285],[-72.690,-37.285],[-72.690,-37.270]], 
        "tipo": "HUMEDAL", "sheet_id": "1x6yAXNNlea3e43rijJu0aqcRpe4oP3BEnzgSgLuG1vU", "pestaña": "Humedales"
    },
    "Pascua Lama (Cordillera)": {
        "coords": [[-70.033,-29.316],[-70.016,-29.316],[-70.016,-29.333],[-70.033,-29.333]], 
        "tipo": "GLACIAR", "sheet_id": "1UTrDs939rPlVIR1OTIwbJ6rM3FazgjX43YnJdue-Dmc", "pestaña": "Mineria"
    }
}

# --- 2. INTERFAZ FRONTAL (SIN SIDEBAR) ---
st.title("🛰️ BioCore V5 Intelligence")
st.write("**Directora:** Loreto Campos Carrasco")

# Sección de Opciones (Antes estaban en el lado, ahora aquí)
st.markdown("### ⚙️ Opciones de Monitoreo")
col_opt1, col_opt2 = st.columns(2)
with col_opt1:
    umbral_dinamico = st.number_input("Umbral Crítico", value=UMBRAL, step=0.05)
with col_opt2:
    st.write(f"**Proyectos activos:** {len(CLIENTES)}")

# Mapa directo en el centro
st.subheader("📍 Ubicación de Proyectos")
m = folium.Map(location=[-35.0, -71.0], zoom_start=5)
for n, i in CLIENTES.items():
    p_fol = [[c[1], c[0]] for c in i['coords']]
    folium.Polygon(locations=p_fol, popup=n, color='green', fill=True, fill_opacity=0.2).add_to(m)
folium_static(m, width=350, height=300)

# Botón de acción principal
st.divider()
ejecutar = st.button("🚀 INICIAR ESCANEO GLOBAL", use_container_width=True)

# --- 3. LÓGICA DE EJECUCIÓN ---
if ejecutar:
    try:
        creds_info = json.loads(st.secrets["gee"]["json"])
        creds = service_account.Credentials.from_service_account_info(creds_info, 
                scopes=['https://www.googleapis.com/auth/spreadsheets', 'https://www.googleapis.com/auth/earthengine'])
        
        if not ee.data._credentials:
            ee.Initialize(creds)
        
        sheets = build('sheets', 'v4', credentials=creds)

        for nombre, info in CLIENTES.items():
            with st.status(f"Analizando {nombre}...", expanded=False):
                p = ee.Geometry.Polygon(info['coords'])
                
                # Procesamiento GEE
                s2 = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED').filterBounds(p).sort('system:time_start', False).first()
                f_rep = datetime.fromtimestamp(s2.get('system:time_start').getInfo()/1000).strftime('%d/%m/%Y')
                
                res = s2.expression('((B8-B4)/(B8+B4+0.5))*1.5', {'B8':s2.select('B8'),'B4':s2.select('B4')}).rename('sa')\
                    .addBands(s2.normalizedDifference(['B3','B8']).rename('nd'))\
                    .reduceRegion(ee.Reducer.mean(), p, 30).getInfo()

                estado = "🟢 NORMAL"
                if res['nd'] < umbral_dinamico: estado = "🔴 ALERTA TÉCNICA"

                # Mostrar Resultados
                st.success(f"**{nombre}**")
                st.write(f"📅 {f_rep} | **Estado:** {estado}")
                st.code(f"SAVI: {res['sa']:.3f}\nND: {res['nd']:.3f}")

                # Sincronizar Google Sheets
                fila = [[f_rep, res['sa'], res['nd'], estado]]
                sheets.spreadsheets().values().append(
                    spreadsheetId=info['sheet_id'], 
                    range=f"{info['pestaña']}!A2", 
                    valueInputOption="USER_ENTERED", 
                    body={'values': fila}).execute()

        st.balloons()
        st.success("✅ Datos actualizados en la nube.")

    except Exception as e:
        st.error(f"Error técnico: {e}")
